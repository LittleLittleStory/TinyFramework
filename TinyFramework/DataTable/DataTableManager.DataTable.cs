    ...
    internal sealed partial class DataTableManager : TinyFrameworkModule, IDataTableManager
    {
        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            public bool HasDataRow(Predicate<T> condition)
            {
                if (condition == null)
                {
                    throw new TinyFrameworkException("Condition is invalid.");
                }
            ...
            public T[] GetDataRows(Comparison<T> comparison)
            {
                if (comparison == null)
                {
                    throw new TinyFrameworkException("Comparison is invalid.");
                }
            ...
            internal override bool AddDataRow(TinyFrameworkSegment<string> dataRowSegment)
            {
                try
                {
                    T dataRow = new T();
                ...
                catch (Exception exception)
                {
                    ...
                    throw new TinyFrameworkException(Utility.Text.Format("Can not parse data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name).ToString(), exception.ToString()), exception);
                }
            }
            ...
            /// <param name="dataRowSegment">要解析的数据表行片段。</param>
            /// <returns>是否增加数据表行成功。</returns>
            internal override bool AddDataRow(TinyFrameworkSegment<byte[]> dataRowSegment)
            {
                try
                {
                    T dataRow = new T();
            ...
            /// <param name="dataRowSegment">要解析的数据表行片段。</param>
            /// <returns>是否增加数据表行成功。</returns>
            internal override bool AddDataRow(TinyFrameworkSegment<Stream> dataRowSegment)
            {
                try
                {
                    T dataRow = new T();
            ...
            private void InternalAddDataRow(T dataRow)
            {
                ...
                {
                    throw new TinyFrameworkException(Utility.Text.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id.ToString(), new TypeNamePair(typeof(T), Name).ToString()));
                }
