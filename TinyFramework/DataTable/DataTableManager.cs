...
namespace TinyFramework.DataTable
{
    ...
    /// 数据表管理器。
    /// </summary>
    internal sealed partial class DataTableManager : TinyFrameworkModule, IDataTableManager
    {
        ...
        public DataTableManager()
        {
            ...
            m_LoadAssetCallbacks = new LoadAssetCallbacks(LoadDataTableSuccessCallback, LoadDataTableFailureCallback,LoadDataTableDependencyAssetCallback);
        ...
        public void SetResourceManager(IResourceManager resourceManager)
        {
            ...
            {
                throw new TinyFrameworkException("Resource manager is invalid.");
            }
        ...
        public void SetDataTableHelper(IDataTableHelper dataTableHelper)
        {
            ...
            {
                throw new TinyFrameworkException("Data table helper is invalid.");
            }
        ...
        public void LoadDataTable(string dataTableAssetName, LoadType loadType, int priority, object userData)
        {
            ...
            {
                throw new TinyFrameworkException("You must set resource manager first.");
            }

            if (m_DataTableHelper == null)
            {
                throw new TinyFrameworkException("You must set data table helper first.");
            }
        ...
        public bool HasDataTable(Type dataRowType)
        {
            if (dataRowType == null)
            {
                throw new TinyFrameworkException("Data row type is invalid.");
            }

            if (!typeof(IDataRow).IsAssignableFrom(dataRowType))
            {
                throw new TinyFrameworkException(Utility.Text.Format("Data row type '{0}' is invalid.", dataRowType.FullName));
            }
        ...
        public void GetAllDataTables(List<DataTableBase> results)
        {
            if (results == null)
            {
                throw new TinyFrameworkException("Results is invalid.");
            }
        ...
        public IDataTable<T> CreateDataTable<T>(string name, string text) where T : class, IDataRow, new()
        {
            ...
            {
                throw new TinyFrameworkException(Utility.Text.Format("Already exist data table '{0}'.", typeNamePair.ToString()));
            }
        ...
        public bool DestroyDataTable<T>(IDataTable<T> dataTable) where T : IDataRow
        {
            if (dataTable == null)
            {
                throw new TinyFrameworkException("Data table is invalid.");
            }
        ...
        {
            IEnumerable<TinyFrameworkSegment<string>> dataRowSegments = null;
            try
            {
            ...
            {
                if (exception is TinyFrameworkException)
                {
                    throw;
                }

                throw new TinyFrameworkException(Utility.Text.Format("Can not get data row segments with exception '{0}'.", exception.ToString()), exception);
            }

            if (dataRowSegments == null)
            {
                throw new TinyFrameworkException("Data row segments is invalid.");
            }

            foreach (TinyFrameworkSegment<string> dataRowSegment in dataRowSegments)
            {
                if (!dataTable.AddDataRow(dataRowSegment))
                {
                    throw new TinyFrameworkException("Add data row failure.");
                }
            }
        }
        ...
        {
            IEnumerable<TinyFrameworkSegment<byte[]>> dataRowSegments = null;
            try
            {
            ...
            foreach (TinyFrameworkSegment<byte[]> dataRowSegment in dataRowSegments)
            {
                if (!dataTable.AddDataRow(dataRowSegment))
                {
                    throw new TinyFrameworkException("Add data row failure.");
        ...
        {
            IEnumerable<TinyFrameworkSegment<Stream>> dataRowSegments = null;
            try
            {
            ...
            foreach (TinyFrameworkSegment<Stream> dataRowSegment in dataRowSegments)
            {
                if (!dataTable.AddDataRow(dataRowSegment))
                {
                    throw new TinyFrameworkException("Add data row failure.");
        ...
        private void LoadDataTableSuccessCallback(string dataTableAssetName, object dataTableAsset, float duration, object userData)
        {
            ...
            {
                throw new TinyFrameworkException("Load data table info is invalid.");
            }
            ...
            try
            {
                ...
                {
                    throw new TinyFrameworkException(Utility.Text.Format("Load data table failure in helper, asset name '{0}'.", dataTableAssetName));
                }
        ...
        private void LoadDataTableFailureCallback(string dataTableAssetName, LoadResourceStatus status, string errorMessage, object userData)
        {
            ...
            throw new TinyFrameworkException(appendErrorMessage);
        }
