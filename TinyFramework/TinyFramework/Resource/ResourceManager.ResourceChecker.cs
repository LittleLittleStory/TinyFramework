...
namespace TinyFramework.Resource
{
    internal sealed partial class ResourceManager : TinyFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceChecker
        {
            ...
            public TinyFrameworkAction<ResourceName, LoadType, int, int, int, int> ResourceNeedUpdate;
            public TinyFrameworkAction<int, int, long, long> ResourceCheckComplete;
            ...
            private void RefreshCheckInfoStatus()
            {
                ...
                foreach (KeyValuePair<ResourceName, CheckInfo> checkInfo in m_CheckInfos)
                {
                    ...
                    if (ci.NeedRemove)
                    {
                        ...
                        {
                            throw new Exception(Utility.Text.Format("Resource '{0}' is not exist in read-write list.", ci.ResourceName.FullName));
                        }
            ...
            private void ParseVersionList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                {
                    throw new Exception(Utility.Text.Format("Version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
                }
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                    {
                        ...
                        {
                            throw new Exception("Version list header is invalid.");
                        }
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            for (int i = 0; i < resourceGroupCount; i++)
                            {
                                ...
                                for (int j = 0; j < resourceGroupResourceCount; j++)
                                {
                                    ...
                                    {
                                        throw new Exception(Utility.Text.Format("Package index '{0}' is invalid, resource count is '{1}'.", index.ToString(), resourceCount.ToString()));
                                    }
                        ...
                        {
                            throw new Exception("Version list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new Exception(Utility.Text.Format("Parse version list exception '{0}'.", exception.ToString()), exception);
                }
                finally
                {
            ...
            private void ParseReadOnlyList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                    {
                        ...
                        {
                            throw new Exception("Readonly list header is invalid.");
                        }
                        ...
                        {
                            throw new Exception("Readonly list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new Exception(Utility.Text.Format("Parse readonly list exception '{0}'.", exception.ToString()), exception);
                }
                finally
                {
                    if (memoryStream != null)
            ...
            private void ParseReadWriteList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                    {
                        ...
                        {
                            throw new Exception("Read-write list header is invalid.");
                        }
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            for (int i = 0; i < resourceCount; i++)
                            {
                                ...
                                {
                                    throw new Exception(Utility.Text.Format("Read-write resource info '{0}' is already exist.", resourceName.FullName));
                                }
                        ...
                        {
                            throw new Exception("Read-write list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new Exception(Utility.Text.Format("Parse read-write list exception '{0}'.", exception.ToString()), exception);
                }
                finally
                {
                    if (memoryStream != null)
            ...
            private void ProcessResourceInfo(ResourceName resourceName, LoadType loadType, int length, int hashCode, bool storageInReadOnly)
            {
                ...
                {
                    throw new Exception(Utility.Text.Format("Resource info '{0}' is already exist.", resourceName.FullName));
                }
