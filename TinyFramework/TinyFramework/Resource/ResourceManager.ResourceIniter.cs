...
namespace TinyFramework.Resource
{
    internal sealed partial class ResourceManager : TinyFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceIniter
        {
            ...
            public TinyFrameworkAction ResourceInitComplete;
            ...
            private void ParsePackageList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                {
                    throw new Exception(Utility.Text.Format("Package list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
                }
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                    {
                        ...
                        {
                            throw new Exception("Package list header is invalid.");
                        }
                        ...
                        {
                            throw new Exception("Package list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new Exception(Utility.Text.Format("Parse package list exception '{0}'.", exception.ToString()), exception);
                }
                finally
                {
