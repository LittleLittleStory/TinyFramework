...
namespace TinyFramework.ObjectPool
{
    internal sealed partial class ObjectPoolManager : TinyFrameworkModule, IObjectPoolManager
    {
        /// <summary>
        /// 对象池。
        /// </summary>
        ...
        {
            private readonly TinyFrameworkMultiDictionary<string, Object<T>> m_Objects;
            ...
            public ObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
                : base(name)
            {
                m_Objects = new TinyFrameworkMultiDictionary<string, Object<T>>();
            ...
            public override int Capacity
            {
                ...
                set
                {
                    if (value < 0)
                    {
                        throw new TinyFrameworkException("Capacity is invalid.");
                    }
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    if (value < 0f)
                    {
                        throw new TinyFrameworkException("ExpireTime is invalid.");
                    }
            ...
            public void Register(T obj, bool spawned)
            {
                if (obj == null)
                {
                    throw new TinyFrameworkException("Object is invalid.");
                }
            ...
            {
                TinyFrameworkLinkedListRange<Object<T>> objectRange = default(TinyFrameworkLinkedListRange<Object<T>>);
            ...
            public void Unspawn(object target)
            {
                if (target == null)
                {
                    throw new TinyFrameworkException("Target is invalid.");
                }
                ...
                {
                    throw new TinyFrameworkException(Utility.Text.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.", new TypeNamePair(typeof(T), Name).ToString(), target.GetType().FullName, target.ToString()));
                }
            }
            ...
            public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
            {
                ...
                {
                    throw new TinyFrameworkException("Release object filter callback is invalid.");
                }
            ...
            public override ObjectInfo[] GetAllObjectInfos()
            {
                ...
                foreach (KeyValuePair<string, TinyFrameworkLinkedListRange<Object<T>>> objectRanges in m_Objects)
                {
            ...
            private void ReleaseObject(T obj)
            {
                ...
                if (internalObject == null)
                {
                    throw new TinyFrameworkException("Can not release object which is not found.");
                }
