namespace TinyFramework.Download
{
    internal sealed partial class DownloadManager : TinyFrameworkModule, IDownloadManager
    {
        ...
        {
            private readonly TinyFrameworkLinkedList<DownloadCounterNode> m_DownloadCounterNodes;
            ...
            public DownloadCounter(float updateInterval, float recordInterval)
            {
                ...
                {
                    throw new TinyFrameworkException("Update interval is invalid.");
                }

                if (recordInterval <= 0f)
                {
                    throw new TinyFrameworkException("Record interval is invalid.");
                }

                m_DownloadCounterNodes = new TinyFrameworkLinkedList<DownloadCounterNode>();
