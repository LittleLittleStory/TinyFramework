...
namespace TinyFramework.Download
{
    ...
    internal sealed partial class DownloadManager : TinyFrameworkModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, int priority, object userData)
        {
            ...
            {
                throw new TinyFrameworkException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new TinyFrameworkException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new TinyFrameworkException("You must add download agent first.");
            }
